...
public class HJH_Inventory : MonoBehaviour
{
    ...
    public void ShowInformation(int buttonIndex)
    {
        ...
        if (buttonIndex >= 0)
        {
            ...
            else if (currentItem.itemName == "주먹도끼")
            {
                itemInfo.text = "주먹에 쥐고 쓸 수 있는 도끼 형태의 뗀석기. 전기 구석기시대의 대표적인 석기이다. 원시인들은 이것을 손에 들고 다용도 도구로 사용했다.";
            }
            else if(currentItem.itemName == "붕어")
            {
                itemInfo.text = "잉어과에 속하는 한국의 어류로 아마도 한국인에게 '담수어류'라고 하면 가장 먼저 붕어를 떠올릴 사람이 많을 정도로 우리에게 친숙한 어류이다.";
            }
            else if (currentItem.itemName == "연어")
            {
                itemInfo.text = "치어는 강에서 태어나 바다로 가서 살다가 성체가 되면 다시 강을 거슬러 올라와 상류에서 알을 낳는 회유성 어종이다.";
            }
            else if (currentItem.itemName == "농어")
            {
                itemInfo.text = "바다에 있으면 배스 민물에 있으면 농어";
            }
